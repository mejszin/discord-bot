$bot.message(start_with: '~project-add') do |event|
    ...
    if words.length > 2
        ...
        result = Projects.new.add_project(owner, title, desc, url)
...
$bot.message(start_with: '~project-join') do |event|
    words = event.content.split(" ")
    if words.length == 2
        result = Projects.new.add_project_member(event.message.user.id.to_s, words[1])
...
$bot.message(start_with: '~project-leave') do |event|
    words = event.content.split(" ")
    if words.length == 2
        result = Projects.new.remove_project_member(event.message.user.id.to_s, words[1])
...
$bot.message(start_with: '~project-enable') do |event|
    words = event.content.split(" ")
    if words.length == 2
        result = Projects.new.set_project_status(event.message.user.id.to_s, words[1], true)
...
$bot.message(start_with: '~project-disable') do |event|
    words = event.content.split(" ")
    if words.length == 2
        result = Projects.new.set_project_status(event.message.user.id.to_s, words[1], false)
...
$bot.message(start_with: '~projects') do |event|
    message = []
    for project in Projects.new.projects do
        if project.active
